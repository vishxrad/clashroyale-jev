...
The light dashboard labels Jev in blue and Laya in burnt orange. Each game feed shows that agent's own perspective, with its troops starting at the bottom. Add `--jev-account YOUR_JEV_NAME --laya-account YOUR_LAYA_NAME` to label the accounts; these names are for display only. Each panel has its own decisions, overlay switches, and video delay selector. The default 5-second video buffer aligns overlays with gameplay and does not delay the bots' actions. Use **Open view** for an individual dashboard with its own start/stop controls.
