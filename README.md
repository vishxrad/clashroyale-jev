...
The light dashboard labels Jev in blue and Laya in burnt orange. Each game feed shows that agent's own perspective, with its troops starting at the bottom. Add `--jev-account YOUR_JEV_NAME --laya-account YOUR_LAYA_NAME` to label the accounts; these names are for display only. Each panel gives most of its space to gameplay. Open **Decisions** below the screen for cards and placement details, or **View** for overlay switches and the video delay selector. These panels open over the video without resizing it. The default 5-second video buffer aligns overlays with gameplay and does not delay the bots' actions. Click an agent's name to open its individual dashboard with its own start/stop controls.
...
Laya's root checkpoint has a 512-token context, so its input keeps brief tactical guidance, the hand and towers plus at most six observed units, prioritizing enemies closest to our side. Troops have at most 12 candidate placements. Spells have at most 11 targets plus a WAIT option, allowing Laya to cancel a spell after inspecting the targets. The guidance says to defend threats, build troop pushes at high elixir, and save spells for useful enemy targets instead of wasting them on a healthy king. These are model instructions, not enforced targeting rules. Laya rejects requests that its tokenizer would truncate. Jev receives the full observed state, action history, card descriptions, and available placement candidates in both single-player and duel modes.
...
Laya is an experimental opponent here. Its authors report weak base-checkpoint performance on unfamiliar typed-decision tasks, and their benchmark-tuned checkpoint was not trained for Clash Royale. A valid card choice does not establish tactical skill. In local replays, Laya still wasted spells on towers despite the corrected guidance and cancel option. The adapter does not replace these choices with scripted tactics. Match format `staged-v3` records this input update; earlier Laya inputs omitted the tactical summary and spell cancellation. Local saved-state probes verify the adapter; a live head-to-head match still requires two prepared accounts.
